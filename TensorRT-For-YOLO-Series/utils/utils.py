...
class BaseEngine(object):
    ...
    def detect_webcam(self, conf=0.2, end2end=False):
...
def vis(img, boxes, scores, cls_ids, conf=0.5, class_names=None):
    for i in range(len(boxes)):
        ...
        if(class_names[cls_id] == "jyunwei"):
            gl = cv2.imread("C:\\Users\\cjweibb\\anaconda3\\envs\\edge_ai_yolov7\\yolov7\\R.png")
            gl = cv2.resize(gl,(50,50))
            gl = cv2.copyMakeBorder(gl,0,img.shape[0]-50,img.shape[1]-50,0,cv2.BORDER_CONSTANT,value=(0,0,0))
            img = cv2.add(img,gl)
